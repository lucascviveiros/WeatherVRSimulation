...
public class WebRequestController : Singleton<WebRequestController>
{
        ...
        foreach (string y in myDeserializedClass.hourly.time)
        {
            ...
            if (customDate == dateHour[0].Replace('-', '/'))
            {
                ...
                if (currentHour == dateHour[1])
                {
                    ...
                    //Debug.Log("Date: " + dateHour[0] + " Hour: " + dateHour[1] + " Temp: " + temp + " Rain: " + humidity + " Cloud: " + cloud + " weather_code: " + weathercode);//+ "ºC");     
                    UIManager.instance.ReturnSliders();
    ...
    /// <summary>
    /// Return a weather condition/state description based on the provided search code.
    /// </summary>
    /// <param name="searchCode">The code to search for.</param>
    /// <returns>The weather state description corresponding to the search code, or an empty string if no match is found.</returns>
    ...
    /// <summary>
    /// Searches the weather from OpenMateoAPI using the provided latitude and longitude coordinates.
    /// </summary>
    ...
    /// <summary>
    /// Coroutine for making a request to the OpenMateo API and processing the response.
    /// </summary>
